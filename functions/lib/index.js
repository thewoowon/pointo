 ...
 */
async function sendNotice(subject, html, tag, replyTo) {
    try {
...
exports.onOpinionCreated = (0, firestore_1.onDocumentCreated)({ document: "feedback/{docId}", region: REGION }, async (event) => {
    ...
    if (!data)
        return;
    const text = String((_b = data.text) !== null && _b !== void 0 ? _b : "").trim();
    if (!text)
        return;
    const uid = String((_c = data.ownerUid) !== null && _c !== void 0 ? _c : "");
    ...
    if (uid) {
        try {
            ...
            firebase_functions_1.logger.warn(`onOpinionCreated: ${uid} 계정 조회 실패`, e);
        }
    }
    // serverTimestamp는 Admin SDK에서 Timestamp로 온다. 트리거가 다시 돌 때를
    // 대비해 값이 없으면 현재 시각으로 채운다.
    const createdAt = (_f = (_e = (_d = data.createdAt) === null || _d === void 0 ? void 0 : _d.toDate) === null || _e === void 0 ? void 0 : _e.call(_d).toISOString()) !== null && _f !== void 0 ? _f : new Date().toISOString();
...
exports.onUserDeleted = (0, firestore_1.onDocumentDeleted)({ document: "users/{docId}", region: REGION }, async (event) => {
    ...
    if (!storeCode) {
        firebase_functions_1.logger.warn(`onUserDeleted: ${docId} — store_code가 없어 로그 정리를 건너뜁니다. ` +
